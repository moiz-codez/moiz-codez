...
Generates:
  assets/stats.svg    hero total + weekly sparkline
  assets/streak.svg   current and longest streak
  assets/langs.svg    top languages, by bytes and by repo count
  assets/year.svg     year contribution map in the portrait's ramp
  assets/hd-*.svg     section headings (about, stack, projects, stats, ecosystem, activity)
  assets/neofetch.svg updates personal stats (repos, stars, commits, followers, etc.)

Env:
  GITHUB_TOKEN  required
  GH_LOGIN      user to summarise (default: moiz-codez)
...
def summarise(user):
    ...
    stars = sum(n.get("stargazerCount", 0) for n in user["repositories"]["nodes"])
    ...
    return dict(
        ...
        repos=repos, stars=stars, commits=total_contributions,
        followers=followers, contribs=len([d for d in days if d["contributionCount"] > 0]))
...
def update_neofetch(path, s):
    """Update the neofetch SVG with live stats from the API."""
    if not os.path.exists(path):
        print(f"neofetch template not found: {path}")
        return False
    ...
    updates = {
        "repo_data": str(s["repos"]),
        "star_data": str(s["stars"]),
        "commit_data": str(s["commits"]),
        "follower_data": str(s["followers"]),
        "contrib_data": str(s["contribs"]),
    }

    changed = False
    for eid, value in updates.items():
        pattern = rf'(id="{eid}"[^>]*>)(.*?)(</text>)'
        new = rf'\1{value}\3'
        updated, n = re.subn(pattern, new, svg, count=1)
        if n:
            changed = changed or (updated != svg)
            svg = updated
...
def main():
    ...
    for word in ("about", "stack", "projects", "stats", "ecosystem", "activity"):
    ...
    # Update neofetch SVG with live stats
    neofetch_path = os.path.join(assets_dir, "neofetch.svg")
    if update_neofetch(neofetch_path, s):
        changed.append("neofetch.svg (stats updated)")
    ...
    print("repos:", s["repos"], "stars:", s["stars"],
